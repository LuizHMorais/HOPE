...
const detectFlowDirection = (rawFlow: RawSheetCell, amount: number): 'inflow' | 'outflow' => {
  ...
  if (flowText) {
    ...
    if (/(out|saida|sa�da|despes|debit|pagamento|withdraw|saque)/.test(flowText)) {
      return 'outflow';
    }
  }
  ...
  const accounts: Account[] = raw.accounts
    .map((record) => {
      ...
      return {
        ...
        institution_name: toStringValue(pickValue(record, ['institution_name', 'institution', 'bank']), 'Institui��o'),
  ...
  const transactions: Transaction[] = raw.transactions
    .map((record) => {
      ...
      return {
        ...
        description: toStringValue(pickValue(record, ['description', 'descricao', 'merchant_name']), 'Transa��o'),
...
export const useGoogleSheetsData = () => {
  ...
  const fetchAllData = useCallback(async (opts?: { force?: boolean; silent?: boolean }) => {
    ...
    try {
      ...
      if (ENV_CONFIG.APP.USE_MOCK_DATA) {
        ...
        try {
          ...
          try {
            ...
            normalized = normalizeDataset(raw);
            apiValid = true;
            warning = 'API Key restrita para este dom�nio. Dados carregados via fallback p�blico (GViz).';
          } catch (gvizError) {
            console.error('[HOPE] Fallback GViz falhou, usando dados de demonstra��o', gvizError);
            normalized = normalizeDataset(buildMockDataset());
            usedMock = true;
            apiValid = false;
            source = 'mock';
            warning = 'N�o foi poss�vel acessar o Google Sheets. Exibindo dados de demonstra��o.';
          }
        }
      }
      ...
      if (!opts?.silent) {
        if (usedMock) {
          toast({
            title: 'Modo demonstra��o',
            description: 'Dados mock carregados.',
          });
        } else {
          toast({
            title: 'Dados atualizados',
            description: `${normalized.people.length} pessoas, ${normalized.accounts.length} contas, ${normalized.transactions.length} transa��es.`,
          });
        }
      }
  ...
  // Função privada para calcular métricas baseadas em transações e contas
  ...
  const owners = data.people.map((p) => ({
    id: p.link_id,
    name: p.person_alias || `${p.first_name} ${p.last_name}`.trim() || 'Usu�rio',
